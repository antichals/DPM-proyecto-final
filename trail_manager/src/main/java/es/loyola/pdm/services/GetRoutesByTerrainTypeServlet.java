...
@WebServlet("/getRoutesByTerrainType")
public class GetRoutesByTerrainTypeServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String terrainType = request.getParameter("terrainType");
        ...
        for (Route route : routeManager.getRoutes()) {
            if (route.getTerrainType().equals(terrainType)) {
                // Convert route to JSON and add to routesJsonArray
